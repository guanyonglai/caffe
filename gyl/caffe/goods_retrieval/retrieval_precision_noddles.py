...
query_num=98 #query图片的张数
for i in range(1,query_num+1):
    name2 = '0000'
    name3 = name2 + str(i)
    name3 = name3[-4:]
    queryImg = caffe.io.load_image('/mnt/disk1/gyl/image_test/product_retrieval/groundtruth_roi0308/laotansuancai/'+name3+'.jpg')
    ...
    for k in range(78,89): #groundtruth的序号
        if rank_ID[0]+1==k:
            touch_num=touch_num+1
            print(rank_ID[i]+1,rank_score[i])#rank_ID[0]从0开始，而我的图片编号从1开始的，故+1
